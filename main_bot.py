...
class TeamView(View):
    def __init__(self):
        ...
        # chunks the roles into blocks of 25 to bypass discord limits
...
async def spawn_teams(ctx):
    # locked to welcome channel only
...
async def leaderboard(ctx):
    ...
    for rank, (cid, state) in enumerate(sorted_teams, 1):
        lines.append(f"{rank}. <#{cid}> - question {state['q_num']}")
...
async def broadcast_start(ctx):
    ...
    for cid in target_channels:
        channel = bot.get_channel(cid)
        if channel:
            game_state[cid] = {"q_num": 1, "waiting_for_img": False}
            file = await get_drive_file("1")
            await channel.send("Game Started! Attach your working image and type the numerical answer in the SAME message.", file=file)
...
async def on_reaction_add(reaction, user):
    ...
    if str(reaction.emoji) == "✅":
        ...
        if target_chan:
            await target_chan.send(f"Verified. Moving to Question {new_q}:", file=file)
